    ...
    match action {
        ...
        WorkoutAction::List { limit, days } => {
            ...
            for w in workouts {
                ...
                rows.push(vec![
                    ...
                    w.workout_type.clone().unwrap_or_default().green().to_string(),
        ...
        WorkoutAction::View { workout_id } => {
            ...
            if let Some(w) = workout {
                ...
                println!("Type: {}", w.workout_type.as_deref().unwrap_or("General").green());
                ...
                for (we, name) in &exercises {
                    ...
                    for s in sets {
                        if s.distance_km.is_some() || s.duration_seconds.is_some() || s.avg_heart_rate_bpm.is_some() {
                ...
                if !cardio_sets.is_empty() {
                    ...
                    for (_, s) in &cardio_sets {
                        ...
                        if let Some(hr) = s.avg_heart_rate_bpm { hr_samples.push(hr); }
                        if let Some(hr) = s.max_heart_rate_bpm { if hr > max_hr { max_hr = hr; } }
                    ...
                    let avg_hr = if hr_samples.is_empty() { 0.0 } else { hr_samples.iter().sum::<f64>() / hr_samples.len() as f64 };
                    let avg_pace = if total_dist > 0.0 { (total_dur as f64 / 60.0) / total_dist } else { 0.0 };

                    let hr_display = if hr_samples.is_empty() && max_hr == 0.0 {
                        "--".to_string()
                    } else {
                        format!("{} / {} bpm", avg_hr.round(), max_hr.round()).red().to_string()
                    };
                    ...
                    print_table(vec!["Total Dist", "Total Time", "Avg Pace", "Avg/Max HR", "Calories"], summary_table);

                    if total_dur > 0 && (aggregated_zones.z1_seconds > 0 || aggregated_zones.z2_seconds > 0 || aggregated_zones.z3_seconds > 0 || aggregated_zones.z4_seconds > 0 || aggregated_zones.z5_seconds > 0) {
                ...
                for (we, name) in exercises {
                    ...
                    for s in &sets {
                        ...
                        if let Some(reps) = s.reps { details.push(format!("{} reps", reps)); }
                        if let Some(weight) = s.weight_kg { details.push(format!("{:.2} kg", weight)); }
                        if let Some(dist) = s.distance_km { details.push(format!("{:.2} km", dist)); }
                        if let Some(dur) = s.duration_seconds { details.push(format_duration(dur as u32)); }
                        if let Some(rpe) = s.rpe { details.push(format!("RPE {}", rpe)); }
                        if let Some(rir) = s.rir { details.push(format!("RIR {}", rir)); }

                        let cardio_info = if s.avg_heart_rate_bpm.is_some() {
                            format!("{} bpm | {} | {} cal", 
                                s.avg_heart_rate_bpm.map(|v| v.round().to_string()).unwrap_or_else(|| "--".to_string()),
                                s.avg_pace_min_per_km.map(format_pace).unwrap_or_else(|| "--".to_string()),
                                s.calories_burned.map(|v| v.to_string()).unwrap_or_else(|| "--".to_string())
                            )
                        } else {
                            "".to_string()
                        };
...
async fn get_workout_summary(repo: &Repository, workout: &crate::models::Workout) -> Result<String> {
    ...
    if cardio_found {
        ...
        Ok(format!("{} • {:.2} km • {} • {} • {} bpm",
            ...
            avg_hr.map(|h| h.round().to_string()).unwrap_or_else(|| "--".to_string())
        ))
    } else {
        Ok(workout.notes.clone().unwrap_or_default().dimmed().to_string())
    }
}
