...
```
Exercise is taken from FIT `session.sport` (e.g. `running`); it must already exist in the catalog. Trackpoints (GPS/HR samples) are stored automatically when present. HR zones are filled from FIT, `--hr-zone-bounds`, or bodylog (DOB + median sleep HR); use `--no-bodylog` to skip. See `docs/import.md`.
