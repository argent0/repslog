...
async fn test_set_numbering() {
    ...
    repo.add_set(we_id, set1_num, Some(10), None, None, None, None, None).await.unwrap();

    let set2_num = repo.get_next_set_number(we_id).await.unwrap();
    assert_eq!(set2_num, 2);
    repo.add_set(we_id, set2_num, Some(12), None, None, None, None, None).await.unwrap();
}
...
async fn test_set_quick_logic() {
    ...
    repo.add_set(we_id, 1, None, None, None, None, None, None).await.unwrap();
