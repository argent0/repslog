...
async fn test_cardio_tracking() {
    ...
    let laps = vec![
        Lap { lap_number: 1, distance_km: 1.0, duration_seconds: 312, pace_min_per_km: 5.2 },
        Lap { lap_number: 2, distance_km: 1.0, duration_seconds: 312, pace_min_per_km: 5.2 },
    ];
    ...
    assert_eq!(s.heart_rate_zones.as_ref().map(|j| j.0.clone()), Some(zones));
