...
async fn test_update_workout() {
    ...
    // Create a workout
    let w_id = repo.create_workout(Some("Push"), None, None, false).await.unwrap();
    
    // Update it
    repo.update_workout(w_id, Some("Pull"), Some("Feeling good"), Some(45), Some(4), Some("2023-01-01"), false).await.unwrap();
...
async fn test_workout_with_custom_date() {
    ...
    let w_id = repo.create_workout(Some("Holiday"), Some("New Year"), Some(custom_date), false).await.unwrap();
...
async fn test_workout_exercise_order() {
    ...
    let ex1 = repo.add_exercise("Ex1", "cat", None, None, None, false, false).await.unwrap();
    let ex2 = repo.add_exercise("Ex2", "cat", None, None, None, false, false).await.unwrap();
    ...
    repo.add_workout_exercise(w_id, ex1, order1, None, false).await.unwrap();
    assert_eq!(order1, 1);

    let order2 = repo.get_max_order_for_workout(w_id).await.unwrap() + 1;
    repo.add_workout_exercise(w_id, ex2, order2, None, false).await.unwrap();
    assert_eq!(order2, 2);
}
