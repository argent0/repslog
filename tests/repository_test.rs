...
async fn test_exercise_management() {
    ...
    let id = repo.add_exercise("Test Pushups", "calisthenics", None, None, None, true, false).await.unwrap();
    assert!(id > 0);
...
async fn test_workout_flow() {
    ...
    // 1. Setup Exercise
    let ex_id = repo.add_exercise("Bench Press", "strength", None, Some("barbell"), None, false, false).await.unwrap();

    // 2. Create Workout
    let w_id = repo.create_workout(Some("Push"), Some("Morning workout"), None, false).await.unwrap();
    assert!(w_id > 0);

    // 3. Add Exercise to Workout
    let we_id = repo.add_workout_exercise(w_id, ex_id, 1, Some("Warmup"), false).await.unwrap();
    assert!(we_id > 0);

    // 4. Add Sets
    let s1 = repo.add_set(we_id, 1, Some(10), Some(60.0), None, None, Some(8.0), None, None, None, None, None, None, None, None, None, None, None, false).await.unwrap();
    let s2 = repo.add_set(we_id, 2, Some(10), Some(60.0), None, None, Some(8.5), None, None, None, None, None, None, None, None, None, None, None, false).await.unwrap();
    assert!(s1 > 0);
    assert!(s2 > 0);
    ...
    // 6. Update Workout
    repo.update_workout(w_id, None, None, Some(45), Some(4), None, false).await.unwrap();
...
async fn test_new_set_fields() {
    ...
    let ex_id = repo.add_exercise("New Fields Exercise", "strength", None, None, None, false, false).await.unwrap();
